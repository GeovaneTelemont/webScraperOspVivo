...
class WebScraperWorker(QThread):
    ...
    def _determinar_tipo_registro(self, categoria, unidade="", descricao=""):
        ...
        if any(x in cat_norm for x in ("material", "materiais", "telefonica")):
            ...
            if any(k in unidade_lower for k in ("m", "u", "cj", "un", "metro", "kg")):
                return "Material"
            elif any(k in descricao_lower for k in ("cfo", "chassi", "conj", "subduto", "material", "cabos", "fibra")):
                return "Material"
            else:
                return "Serviço"
        ...
        try:
            ...
            for servico_idx, servico in enumerate(serviços):
                ...
                try:
                    ...
                    sleep(3)
                    self.message.emit(f"✅ ID {id_value}: Serviço {servico_idx+1} - Memória de Cálculo")
                except:
                    self.message.emit(f"⚠️ ID {id_value}: Serviço {servico_idx+1} - Não encontrou Memória de Cálculo")
                    if len(serviços) > 1:
                        page.go_back()
                        sleep(5)
                    continue
                ...
                while True:
                    ...
                    if tabela.count() > 0:
                        ...
                        for linha in linhas:
                            ...
                            if len(celulas) >= 9:
                                ...
                                linha_completa = [id_value] + valores
                                todos_dados.append(linha_completa)
                        
                        self.message.emit(f"   ✓ Extraiu {len(linhas)} linhas da página {pagina}")
                    else:
                        ...
                        try:
                            ...
                            if tabela_alt.count() > 0:
                                ...
                                for linha in linhas:
                                    celulas = linha.locator('td').all()
                                    if len(celulas) >= 9:
                                        valores = [c.text_content().strip().replace('R$', '') for c in celulas]
                                        while len(valores) < 9:
                                            valores.append('')
                                        todos_dados.append([id_value] + valores)
                                self.message.emit(f"   ✓ Extraiu {len(linhas)} linhas (tabela alternativa)")
                        except:
                            pass
                    ...
                    tem_proxima = False
                    try:
                        next_btns = page.locator('//li[not(contains(@class, "disabled"))]//a[@aria-label="Next"]')
                    ...
                    try:
                        page.locator('//li[not(contains(@class, "disabled"))]//a[@aria-label="Next"]').first.click()
                        sleep(3)
                        pagina += 1
                    except Exception as e:
            ...
            self.message.emit(f"✅ ID {id_value}: Finalizado - {len(todos_dados)} linhas extraídas")
            return todos_dados
                ...
                if not self._is_logged(page):
                    ...
                    if self.username and self.password:
                        try:
                            ...
                            self.message.emit("✅ Usuário e senha preenchidos. Complete o CAPTCHA manualmente...")
                        except:
                            self.message.emit("ℹ️ Complete o login manualmente (CAPTCHA)...")
                    
                    # Aguardar login manual
                    page.wait_for_selector(self.logged_selector, timeout=180000, state="visible")
    ...
    def _scrap_draft(self, page, df):
        ...
        arquivo = self.download_path / "osp_vivo_draft.xlsx"
        colunas = ["ID", "TIPO DE REGISTRO", "CÓDIGO", "DESCRIÇÃO", 
        ...
        for idx, row in df.iterrows():
            ...
            self.message.emit(f"📋 Processando ID {id_value} ({idx + 1}/{total_ids})...")
            
            try:
                dados = self._pesquisar_id_draft(page, id_value)
                if dados:
                    resultados.extend(dados)
                    self.message.emit(f"✅ ID {id_value} extraído ({len(dados)} linhas)")
                else:
    ...
    def _scrap_medicao(self, page, df):
        ...
        arquivo = self.download_path / "osp_vivo_medicao.xlsx"
        colunas = ["ID", "TIPO DE REGISTRO", "CÓDIGO", "DESCRIÇÃO", 
                   "QUANTIDADE", "PREÇO UNITÁRIO", "UNIDADE", "PREÇO TOTAL", "CATEGORIA"]
        resultados = []
    ...
    def _scrap_id_cancelado(self, page, df):
        ...
        arquivo = self.download_path / "osp_id_cancelado.xlsx"
        ...
        for idx, row in df.iterrows():
            ...
            try:
                ...
                if dados:
                    ...
                    self.message.emit(f"✅ ID {id_value}: Contrato='{dados[1]}', OSP='{dados[2]}'")
                else:
    ...
    def _scrap_memoria_calculo_main(self, page, df):
        ...
        # Define colunas
        colunas = [
            "ID", "CLASSE", "CODIGO", "DESCRIÇÃO DO SERVIÇO", 
            "UNIDADE", "PONTOS", "CUSTO UNITÁRIO (R$)", 
            "QUANTIDADE EXECUTADA", "PONTOS TOTAIS", "CUSTO TOTAL (R$)"
        ]
...
class MainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.mode_memoria.setToolTip("Extrai tabela específica de memória de cálculo com:\n- Classe, Código, Descrição\n- Pontos, Custos Unitário e Total\n- Quantidade Executada")
